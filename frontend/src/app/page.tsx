...
const EXPLORER_PROJECTS = [
  { symbol: "ROUTE", name: "Router Protocol", score: 63, reason: "ECONOMIC FAILURE", velocity: "Accelerating", halfLife: "15 days", sustainability: "0.20", trend: [-10, -20, -50, -65, -80], icon: "https://s2.coinmarketcap.com/static/img/coins/64x64/32544.png", drawdown: "-87.3%", summary: "Revenue fails to cover infrastructure costs, causing a liquidity spiral and heavy reliance on inflationary emissions." },
      ...
      <main className="max-w-[1400px] mx-auto p-4 md:p-6 mt-2 relative z-10">
        ...
        {result && !loading && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-700">
            ...
            {/* TAB NAVIGATION */}
              <div className="flex gap-2 border-b border-[#333333] px-2">
                {['overview', 'evidence', 'methodology', 'ai_terminal'].map((tab) => (
                <button 
                  key={tab}
              ...
              <div className="border border-[#333333] rounded-none bg-[#1c1d1c] p-8 animate-in fade-in slide-in-from-right-8 duration-500">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-6">
                    <div>
                      <h4 className="text-sm font-bold text-[var(--color-winter-green)] mb-3 tracking-widest uppercase font-blender">Data Sources</h4>
                      <p className="text-sm text-[#ececec] font-sans leading-relaxed">
                        Data is aggregated via the <span className="font-mono text-white bg-white/10 px-1 rounded">CoinMarketCap API</span>. We fetch identity mappings, live quotes, historical order-book depth, and on-chain DEX metrics (liquidity pairs, holder counts, honeypot analysis, buy/sell taxes).
                      </p>
                    </div>

                    <div>
                      <h4 className="text-sm font-bold text-[var(--color-winter-green)] mb-3 tracking-widest uppercase font-blender">Raw Telemetry Sourced</h4>
                          ...
                          <div key={idx} className="bg-[#252725] p-3 border border-[#333333] flex flex-col">
                            ...
                            <span className="text-sm font-blender text-white">
                              {typeof val === 'number' && key.includes('percent') 
                                ? val.toFixed(2) + '%' 
                                : typeof val === 'number' 
                                  ? (val > 1000 ? val.toLocaleString(undefined, {maximumFractionDigits: 0}) : val.toLocaleString())
                                  : String(val)}
                            </span>
                          </div>
                        ))}
                  ...
                  <div className="space-y-6">
                    <div>
                      <h4 className="text-sm font-bold text-[var(--color-winter-green)] mb-3 tracking-widest uppercase font-blender">Correlation Analysis Engine</h4>
                      <p className="text-sm text-[#888888] font-sans leading-relaxed mb-4">
                        The Autopsy Score Engine performs a 3-Layer diagnostic analysis to compute the final Death Score. We do not rely on isolated metrics; instead, we analyze structural relationships between momentum, depth, and sentiment.
                      </p>
                      
                      <ul className="space-y-4">
                        <li className="bg-[#252725] p-4 border-l-2 border-[var(--color-winter-purple)]">
                          <h5 className="font-bold text-white text-sm mb-1 uppercase tracking-wider font-blender">1. Market Dynamics (Volume/Mcap Ratio)</h5>
                          <p className="text-xs text-[#888888]">We cross-reference 24h trading volume against Market Capitalization. A chronically low ratio indicates heavy supply overhang, wash trading, and 'ghost chain' properties.</p>
                        </li>
                        <li className="bg-[#252725] p-4 border-l-2 border-[var(--color-winter-green)]">
                          <h5 className="font-bold text-white text-sm mb-1 uppercase tracking-wider font-blender">2. Liquidity Exhaustion</h5>
                          <p className="text-xs text-[#888888]">Liquidity depth is evaluated relative to market cap. If less than 1% of the asset's value is backed by actual DEX/CEX liquidity, a death spiral is imminent upon any sell pressure.</p>
                        </li>
                        <li className="bg-[#252725] p-4 border-l-2 border-[#ff3333]">
                          <h5 className="font-bold text-white text-sm mb-1 uppercase tracking-wider font-blender">3. Structural Decay (Drawdown Matrix)</h5>
                          <p className="text-xs text-[#888888]">We evaluate continuous drawdown over 7d, 30d, 60d, and 90d periods. Persistent decay without mean reversion confirms terminal holder capitulation.</p>
                        </li>
                      </ul>
                    </div>
                  </div>
